...
bool InvokeThunderAddLink(NPObject* obj, const NPVariant* args,
	...
	if (argCount == 0) {
		...
		HRESULT hr = ::CoInitialize(0);
		IAgent *pAgent;
		hr = Utils::GetCom(__uuidof(Agent), 
				__uuidof(IAgent), (void**)&pAgent); 
		if(FAILED(hr)) {
			flag = false;
		}
		else {
			hr = pAgent->AddTask(NPVARIANT_TO_STRING(args[0]).UTF8Characters, "", "",
				Utils::Utf8ToUnicode((char*)NPVARIANT_TO_STRING(args[1]).UTF8Characters),
		...
		if(FAILED(hr)) {
			...
			for(int i = 2 ; i < count ; i += 2) {
				if(NPVARIANT_IS_STRING(args[i]) && NPVARIANT_IS_STRING(args[i - 1])) {
					hr = pAgent->AddTask(NPVARIANT_TO_STRING(args[i - 1]).UTF8Characters, "", "",
